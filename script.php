...
function changePos(nowId, dice){
    if(Before updating Positions In Database){
    ...
    //From GIT{
        ...
        let playerDivPrev = wholeGrid.querySelector(`.p2`);
        playerDivPrev.style.transition = 'background-color 1s ease';
        playerDivPrev.style.backgroundColor = 'rgba(0, 0, 0, 0)';

        nextpos = pos + dice;
        wholeGrid = document.getElementById(`grid_${ nextpos }`);
        let newPlayer = wholeGrid.querySelector(`.p2`);
        if (newPlayer) {
            newPlayer.style.transition = 'background-color 1s ease';
            if (nowId == 1)
            newPlayer.style.backgroundColor = 'red';
        else if (nowId == 2)
            newPlayer.style.backgroundColor = 'green';
            ...
            console.log(`.p1 - ${ nextpos } not found in grid_${ nextpos }`);
            pos = nextpos;
            if (nextpos == 100)
                alert(`You Won`);
            else if (nextpos > 100)
